...
static constexpr int TEX_BITS = 6;
...
struct raycaster_t
{
	...
	template <typename T>
	void render(const camera_t &cam, const std::vector<std::unique_ptr<T>> &sprites, pixmap_t &backbuffer, viewport_t vp = {}, int detail = 2);
...
template <typename T>
void raycaster_t::render(const camera_t &cam, const std::vector<std::unique_ptr<T>> &sprites, pixmap_t &backbuffer, viewport_t vp, int detail)
{
