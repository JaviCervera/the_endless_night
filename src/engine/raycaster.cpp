...
// Always use textures which are 64x64 on walls and floors or you'll get unexpected results
static constexpr int TEX_SIZE = 64;
static constexpr int TEX_BITS = 6; // log2(TEX_SIZE)
			...
			map_size(map_size)
{
	...
	// Example sprites placed in open cells of the world map.
	// fpg_idx is a 0-based FPG map index. Palette color 0 = transparent.
	sprites = {
			{vec2_t{real_t(20.5f), real_t(11.5f)}, 53},
			{vec2_t{real_t(13.5f), real_t(11.5f)}, 55},
			{vec2_t{real_t(7.5f), real_t(1.5f)}, 57},
			{vec2_t{real_t(17.5f), real_t(2.5f)}, 58},
	};
}
